...
router.post('/users', auth.middleware.isLoggedIn,  validators.createUser, (req, res, next) => { // TODO: email/captcha validation
  ...
  if (auth.predicates.isProvinsi(req.user)) {
    ...
    if(!req.body.username) req.body.username = usernameGenerator('provinsi', req.body.nama);
  ...
  return queries.createUser(req.body, req.user.username)
    .then((insertedUser) => {
...
router.patch('/users/:username?', auth.middleware.isAdmin, validators.updateUser, (req, res, next) => {
  ...
  if(req.params.username) {
    return queries.updateUser(req.params.username, userUpdates)
    .then((affectedRowCount) => {
      return res.json({ affectedRowCount: affectedRowCount });
    })
    .catch(next);
  } else {
    return queries.updateUser(req.user.username, userUpdates)
    .then((affectedRowCount) => {
      return res.json({ affectedRowCount: affectedRowCount});
    })
    .catch(next);
